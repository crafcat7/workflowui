...
TEST(RpcHandlerTest, SuccessfulRequestEchoesIdAndResult) {
  ...
  auto resp =
      response_of(rpc, R"({"jsonrpc":"2.0","id":"abc","method":"add","params":{"a":2,"b":3}})");
...
TEST(RpcHandlerTest, MethodExceptionBecomesMinus32000) {
  RpcHandler rpc;
  rpc.register_method("boom", [](const json&) -> json { throw std::runtime_error("kaboom"); });
...
TEST(RpcHandlerTest, NotificationExceptionIsSwallowed) {
  ...
  RpcHandler rpc;
  rpc.register_notify("crash", [](const json&) { throw std::runtime_error("intentional"); });
  std::string s;
