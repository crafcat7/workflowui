...
void crash_handler(int sig) {
  ...
  int hl = std::snprintf(hdr, sizeof(hdr), "\n[CRASH] signal %d (%s), backtrace (%d frames):\n",
                         sig, name ? name : "?", n);
  if (hl > 0)
    (void)!write(STDERR_FILENO, hdr, static_cast<size_t>(hl));
...
int main(int argc, char* argv[]) {
  ...
  auto print_help = []() {
    std::cout << "Usage: workflow_backend [PORT] [options]\n"
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--port") {
      ...
      if (i + 1 >= argc) {
        std::cerr << "--shared-dir requires a value\n";
        return 1;
      }
      shared_dir = argv[++i];
    } else if (arg == "--allow-origin") {
      if (i + 1 >= argc) {
        std::cerr << "--allow-origin requires a value\n";
        return 1;
      }
  ...
  if (origin_flag_seen) {
    for (const auto& o : allow_origins)
  ...
  std::cout << "[Backend] Allowed origins: " << (sec.has_origin_allowlist() ? "configured" : "ANY")
            << "\n";
  ...
  rpc.register_method("workflow.execute", [&](const json& params) -> json {
    ...
    for (auto& jn : params["nodes"]) {
      ...
      if (!jn.contains("type") || !jn["type"].is_string() ||
          jn["type"].get<std::string>().empty()) {
    ...
    if (params.contains("breakpoints")) {
      ...
      for (auto& bp : params["breakpoints"]) {
        if (bp.is_string())
          breakpoints.push_back(bp.get<std::string>());
      }
    }
  ...
  rpc.register_notify("workflow.stop", [&](const json&) { executor->stop(); });
  ...
  rpc.register_method("workflow.state",
                      [&](const json&) -> json { return executor->snapshot_state(); });

  rpc.register_notify("debug.continue",
                      [&](const json&) { executor->debug_controller().resume(); });

  rpc.register_notify("debug.step_over",
                      [&](const json&) { executor->debug_controller().step_over(); });

  rpc.register_method("debug.add_breakpoint", [&](const json& params) -> json {
    if (!params.is_object() || !params.contains("node_id") || !params["node_id"].is_string() ||
        params["node_id"].get<std::string>().empty()) {
