...
std::optional<float> apply_selector(const std::string& selector, const TensorData& t) {
  ...
  if (selector == "sum")
    return std::accumulate(t.begin(), t.end(), 0.0f);
  if (selector == "mean") {
...
std::optional<float> scalar_from_input(const std::string& selector, const PortValue& input) {
  if (auto* t = std::get_if<TensorData>(&input))
    return apply_selector(selector, *t);
  if (auto* f = std::get_if<float>(&input))
    return *f;
  if (auto* i = std::get_if<int64_t>(&input))
...
bool split_comparison(const std::string& expr, std::string& lhs, std::string& op,
                      std::string& rhs) {
...
bool evaluate_condition(const std::string& raw_expr, const PortValue& input,
  ...
  if (split_comparison(expr, lhs, op, rhs)) {
    float rhs_val;
    if (!try_parse_float(rhs, rhs_val))
      return fail("rhs is not a number");
    auto scalar = scalar_from_input(lhs, input);
    if (!scalar)
      return fail("selector could not be applied to input");
  ...
  if (!scalar)
    return fail("unrecognised selector or empty input");
  return *scalar != 0.0f;
}
