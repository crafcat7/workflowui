...
nlohmann::json parse_array(const std::string& payload, const std::string& layer_name, int key) {
    ...
    for (int i = 1; i <= count; ++i) arr.push_back(parse_scalar(parts[i]));
    return arr;
}
...
ModelGraph NcnnInspector::inspect(const ModelInspectRequest& req) {
    ...
    for (int li = 0; li < layer_count; ++li) {
        ...
        auto tokens = split_ws(line);
        if (tokens.empty()) { --li; continue; } // skip blank lines
        if (tokens.size() < 4) {
        ...
        for (int i = 0; i < in_count; ++i) layer.input_blobs.push_back(tokens[4 + i]);
        for (int i = 0; i < out_count; ++i) layer.output_blobs.push_back(tokens[4 + in_count + i]);
        ...
        for (size_t ti = expected; ti < tokens.size(); ++ti) {
            ...
            int key = 0;
            try { key = std::stoi(kv.substr(0, eq)); }
            catch (const std::exception&) {
