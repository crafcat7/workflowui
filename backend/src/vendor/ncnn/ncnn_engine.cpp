...
NetHandle NcnnEngine::init_net(const NetConfig& config) {
  ...
  if (config.num_threads > 0)
    entry->net.opt.num_threads = config.num_threads;
  ...
  if (config.empty_weights) {
    ...
    if (!config.model_path.empty()) {
      ...
      if (entry->net.load_model(dr) != 0) {
        throw std::runtime_error("NcnnEngine: failed to synthesize zero weights for " +
                                 config.param_path);
      }
    }
  } else {
    if (config.model_path.empty()) {
      throw std::runtime_error(
          "NcnnEngine::init_net: modelPath is required unless emptyWeights=true");
    }
    if (entry->net.load_model(config.model_path.c_str()) != 0) {
      throw std::runtime_error("NcnnEngine: failed to load .bin: " + config.model_path);
    }
...
void NcnnEngine::configure(NetHandle handle, const NetConfig& config) {
  ...
  if (!entry)
    throw std::runtime_error("NcnnEngine::configure: unknown handle");
...
ncnn::Mat make_input_mat(const TensorData& input, const NetConfig& cfg) {
  ...
  const size_t expected = (w > 0 && h > 0 && c > 0) ? static_cast<size_t>(w) * h * c
  ...
  if (w > 0 && h > 0 && c > 0)
    ...
    mat.create(static_cast<int>(input.size()));
...
InferResult NcnnEngine::execute(NetHandle handle, const TensorData& input) {
  auto entry = get(handle);
  if (!entry)
    throw std::runtime_error("NcnnEngine::execute: unknown handle");
  ...
  auto start = std::chrono::steady_clock::now();
  {
    ...
    if (ex.extract(entry->cfg.output_name.c_str(), out) != 0) {
      throw std::runtime_error("NcnnEngine: output blob '" + entry->cfg.output_name +
                               "' not found");
    }
  }
...
BenchmarkResult NcnnEngine::benchmark(NetHandle handle, const TensorData& input, int duration_sec,
  ...
  auto entry = get(handle);
  if (!entry)
    throw std::runtime_error("NcnnEngine::benchmark: unknown handle");
  if (duration_sec <= 0)
    duration_sec = 10;
