...
WsServer::WsServer(int port, RpcHandler& handler) : port_(port), handler_(handler) {}
...
void WsServer::run() {
      ...
      {.compression = uWS::DISABLED,
       ...
       .upgrade =
           [](auto* res, auto* req, auto* context) {
             ...
             res->template upgrade<PerSocketData>(PerSocketData{}, wsKey, wsProto, wsExt, context);
           },
       .open =
           [](auto* ws) {
       ...
       .close = [](auto*, int, std::string_view) { std::cout << "[WS] Client disconnected\n"; }});
  ...
  // half-destroyed publisher.
  {
    ...
    publish_fn_ = [&app](std::string msg) { app.publish(BROADCAST_TOPIC, msg, uWS::OpCode::TEXT); };
  }
...
void WsServer::broadcast(const std::string& method, const json& params) {
  ...
  if (loop && fn) {
    ((uWS::Loop*)loop)->defer([fn = std::move(fn), msg = std::move(msg)]() { fn(msg); });
  }
}
